...
correct_cities = ['Новосибирск', 'Красноярск', 'Курск', 'Кемерово', 'Оренбург', 'Гамбург', 'Ижевск', "Омск", "Москва"]
players_in_game = []
active_games = []
        ...
        if is_message(event):
            if is_play_game(event):
                ...
                if not igra.is_unused_city(city):
                    send_message(user1, "Такой город уже был назван, ti proigral")
                    send_message(user2, "Ты выйграл т.к твой противник проиграл")
                    kill_game(igra)

                do_step(user1, city)
                send_message(user2, f"Твой ход. Ходишь на букву: {igar.last_char}.\n"
